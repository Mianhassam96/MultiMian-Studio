...
const SOCIAL_PATHS = {
  ...
  LinkedIn: 'M4.98 3.5C4.98 4.88 3.9 6 2.5 6S0 4.88 0 3.5 1.07 1 2.5 1 4.98 2.12 4.98 3.5zM.5 8h4V24h-4zM8.5 8h3.8v2.18h.06c.53-1 1.82-2.06 3.75-2.06 4 0 4.74 2.64 4.74 6.07V24h-4V15.5c0-2.04-.04-4.66-2.84-4.66-2.84 0-3.28 2.22-3.28 4.5V24h-4z',
  Facebook: 'M22 12c0-5.522-4.477-10-10-10S2 6.478 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.99H7.898v-2.888h2.54V9.797c0-2.507 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.242 0-1.63.771-1.63 1.562v1.875h2.773l-.443 2.888h-2.33v6.99C18.343 21.128 22 16.991 22 12z',
...
const SOCIALS = [
  ...
  { name: 'Twitter',  href: 'https://twitter.com/mianhassam96'              },
]
      ...
      <div className="container pt-16 pb-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
          ...
          <div className="lg:col-span-1">
            ...
            <p className="text-muted text-sm leading-relaxed mb-5">
              A full-stack portfolio showcasing modern web experiences, clean code, and interactive UIs built with Next.js, React, and Tailwind CSS.
            </p>
              ...
              {SOCIALS.map((s) => (
                ...
                >
                  <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4" aria-hidden>
                    <path d={SOCIAL_PATHS[s.name as keyof typeof SOCIAL_PATHS]} />
                  </svg>
                </motion.a>
              ))}
            </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {QUICK_LINKS.map(({ href, label }) => (
                <li key={href}>
                  <Link href={href} className="text-sm text-muted hover:text-primary transition-colors duration-200 inline-flex items-center gap-1.5 group">
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {RESOURCES.map(({ href, label, external }) => (
                <li key={href}>
                  {external ? (
                    <a href={href} target="_blank" rel="noopener noreferrer" className="text-sm text-muted hover:text-primary transition-colors duration-200 inline-flex items-center gap-1.5 group">
                      <span className="w-1 h-1 rounded-full bg-primary/30 group-hover:bg-primary transition-colors" aria-hidden />
                      {label}
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-muted">
              ...
              <li className="flex items-center gap-2">
                <span className="text-base" aria-hidden>⭐</span>
                <a href="https://github.com/Mianhassam96/MultiMian-Studio" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline font-medium">
                  Star on GitHub
                </a>
              </li>
            </ul>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-muted">
          <p>
            ...
            <span className="font-semibold text-primary">MultiMian Studio</span>
            {' — Built by '}
            <a href="https://github.com/Mianhassam96" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline font-medium">
              Mian Hassam
            </a>
          </p>
