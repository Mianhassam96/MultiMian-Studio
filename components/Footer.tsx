...
const SERVICE_LINKS = [
  { href: '/services#business-websites',        label: 'Business Website Development' },
  { href: '/services#ecommerce-solutions',      label: 'E-Commerce Development'       },
  { href: '/services#web-applications',         label: 'Custom Web Applications'      },
  { href: '/services#maintenance-optimization', label: 'Website Maintenance'          },
...
const borderStyle = { borderColor: 'var(--footer-border)' }
    ...
    <footer style={footerStyle} role="contentinfo">
        ...
        <div className="container py-14">
          <div className="flex flex-col md:flex-row items-center justify-between gap-8">
            <div>
              ...
              <p style={{ color: 'var(--footer-text)' }} className="text-base">
            ...
            <div className="flex flex-col sm:flex-row gap-3 shrink-0">
              <Link
                href="/contact"
                className="px-7 py-3.5 bg-primary hover:bg-primary-dark text-white rounded-xl font-semibold text-sm transition-all duration-200 shadow-lg hover:-translate-y-0.5 text-center"
              >
              ...
              <a
                ...
                className="px-7 py-3.5 rounded-xl font-semibold text-sm transition-all duration-200 hover:-translate-y-0.5 text-center text-white"
                style={{ backgroundColor: 'var(--footer-cta-bg)', border: '1px solid var(--footer-border)' }}
              >
                💬 WhatsApp Us
              </a>
            </div>
      ...
      <div className="border-b" style={borderStyle}>
        <div className="container py-5">
          <div className="flex flex-wrap items-center justify-center gap-6 md:gap-10 text-sm" style={{ color: 'var(--footer-text)' }}>
            {TRUST_BADGES.map((b) => (
      ...
      <div className="container py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-10 mb-14">
          ...
          <div className="lg:col-span-2">
            ...
            {/* Logo — matches navbar, text-only */}
            <Link href="/" className="flex items-baseline gap-0 mb-5 w-fit group">
              <span
                className="font-black text-2xl leading-none bg-gradient-to-r from-primary to-primary-dark bg-clip-text text-transparent"
            ...
            <p className="text-sm leading-relaxed mb-6 max-w-xs" style={{ color: 'var(--footer-text)' }}>
              Professional website development for businesses. Modern, fast, SEO-optimized websites that help you establish credibility, attract customers, and grow online.
            </p>
          ...
          {/* Quick Links */}
          <div>
            <h3 className="text-xs font-bold uppercase tracking-widest mb-5 text-white/40">Quick Links</h3>
            <ul className="space-y-3">
              {QUICK_LINKS.map(({ href, label }) => (
                <li key={href}>
                  <Link href={href} className="text-sm flex items-center gap-2 group transition-colors hover:text-primary" style={{ color: 'var(--footer-text)' }}>
                    <span className="w-1 h-1 rounded-full bg-white/20 group-hover:bg-primary transition-colors flex-shrink-0" aria-hidden />
                    {label}
                  </Link>
                </li>
              ))}
          ...
          {/* Services */}
          <div>
            <h3 className="text-xs font-bold uppercase tracking-widest mb-5 text-white/40">Services</h3>
            <ul className="space-y-3">
          ...
          {/* Resources */}
          <div>
            <h3 className="text-xs font-bold uppercase tracking-widest mb-5 text-white/40">Resources</h3>
            <ul className="space-y-3 mb-6">
              {RESOURCES.map(({ href, label, external }) => (
                <li key={href}>
                  {external ? (
                    <a href={href} target="_blank" rel="noopener noreferrer"
                      className="text-sm flex items-center gap-2 group transition-colors hover:text-primary"
                      style={{ color: 'var(--footer-text)' }}
                    >
                      <span className="w-1 h-1 rounded-full bg-white/20 group-hover:bg-primary transition-colors flex-shrink-0" aria-hidden />
                      {label}
                      <svg className="w-3 h-3 opacity-40" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg text-xs font-semibold transition-all duration-200 hover:text-white hover:border-primary/50"
              style={{ backgroundColor: 'var(--footer-cta-bg)', border: '1px solid var(--footer-border)', color: 'var(--footer-text)' }}
            >
        ...
        {/* ── Bottom bar ── */}
        <div className="pt-8 border-t flex flex-col md:flex-row items-center justify-between gap-4 text-xs" style={{ borderColor: 'var(--footer-border)', color: 'var(--footer-text)' }}>
          <p>
            © {year}{' '}
            <span className="font-semibold text-white">MultiMian Studio</span>
            {' '}— Built by{' '}
            <a href="https://github.com/Mianhassam96" target="_blank" rel="noopener noreferrer" className="text-primary hover:text-primary-dark transition-colors font-medium">
              Mian Hassam
            </a>
          </p>
